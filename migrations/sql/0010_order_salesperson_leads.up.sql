-- =============================================================================
-- 0010 The salesperson named on the order leads it (CFO, 2026-09-25):
--   "It will be whichever salesperson is named on the Sales Order email, as they will have led
--    the opportunity."
--   1. NN/E is judged for the order's salesperson: was the customer pre-existing when THAT
--      salesperson first took the account? (Before: the account's current owner.) With no
--      salesperson on the order, the account owner on the order date is used (House -> E).
--   2. On approval, ownership passes to the order's salesperson from the order date, so account
--      history keeps itself current. Conflicts (a later ownership change already recorded, or an
--      inactive salesperson) are logged to account_history_review, never forced.
-- =============================================================================
...
UPDATE sales.exception_rule SET introduced_in = '0010',
       description = 'Order salesperson differs from the account owner; on approval the account passes to them (warning)'
 WHERE rule_code = 'SALESPERSON_NOT_ACCOUNT_OWNER';
UPDATE sales.exception_rule SET introduced_in = '0010',
       description = 'NN/E suffix disagrees with whether the customer pre-existed the order''s salesperson taking the account (warning)'
 WHERE rule_code = 'REPORTING_CATEGORY_MISMATCH';
UPDATE sales.exception_rule SET introduced_in = '0010',
       description = 'NN/E-coded order with no salesperson and no account owner, so it cannot be verified (warning)'
 WHERE rule_code = 'NO_ACCOUNT_ALLOCATION';

-- Ownership follows the salesperson who led an approved order.
CREATE FUNCTION sales.take_account_for_order(p_sales_order_id bigint) RETURNS text
LANGUAGE plpgsql AS $$
DECLARE
    o          record;
...
CREATE FUNCTION sales.tg_sales_order_take_account() RETURNS trigger
LANGUAGE plpgsql AS $$
BEGIN
    PERFORM sales.take_account_for_order(NEW.sales_order_id);
    RETURN NEW;
END
$$;
CREATE TRIGGER take_account_on_approval
    ...
    EXECUTE FUNCTION sales.tg_sales_order_take_account();
